...
object HttpErrorHandlerExceptions {
   ...
   */
  def throwableToUsefulException(sourceMapper: Option[SourceMapper], showDevErrors: Boolean, throwable: Throwable): UsefulException = throwable match {
    case useful: UsefulException => useful
    case e: ExecutionException => throwableToUsefulException(sourceMapper, showDevErrors, e.getCause)
    case prodException if !showDevErrors => UnexpectedException(unexpected = Some(prodException))
    case other =>
